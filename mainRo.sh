...
move_to_cloud() {
    read -p "Introduceți fișierele pentru încărcare (separate prin spații): " files
    read -p "Introduceți URL-ul depozitului Git: " repo_url
    local temp_dir
    temp_dir=$(mktemp -d)

    log_debug "Se clonează depozitul: $repo_url în $temp_dir"
    git clone "$repo_url" "$temp_dir" || {
        echo "Eroare la clonarea depozitului." >&2
        return
    }

    log_debug "Se copiază fișierele în $temp_dir"
    cp $files "$temp_dir" || {
        echo "Eroare la copierea fișierelor în directorul depozitului." >&2
        return
    }

    (cd "$temp_dir" && git add . && git commit -m "Adaugă fișiere de backup" && git push) || {
        echo "Eroare la trimiterea fișierelor în depozit." >&2
        return
    }

    rm -rf "$temp_dir"
    echo "Fișierele au fost încărcate cu succes în cloud!"
}
